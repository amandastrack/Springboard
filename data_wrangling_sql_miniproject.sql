...
SELECT z.facname, z.name, z.cost
FROM (SELECT f.name as facname,
      ...
      LEFT JOIN Facilities AS f ON b.facid = f.facid
      LEFT JOIN Members AS m ON b.memid = m.memid
      WHERE (b.starttime LIKE '2012-09-14%') ) as z
HAVING cost > 30
ORDER BY cost DESC
...
FROM (SELECT f.name as name, CASE WHEN b.memid = 0 THEN f.guestcost * b.slots ELSE f.membercost * b.slots END AS revenue FROM Bookings AS b
LEFT JOIN Facilities AS f ON b.facid = f.facid) as z
GROUP BY z.name
HAVING revtotal < 1000
